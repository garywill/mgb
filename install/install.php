	...
	if(isset($_SESSION['install_language'])) {
		...
		if(empty($_POST['step'])) {
			...
			if(isset($_POST['eula_agreement']) AND $_POST['eula_agreement'] == 1) {
				...
				// does the mysql version fit?
				switch(version_compare('4.1.13', $mysql_version)) {
			...
			if(!isset($_POST['sent'])) {
				...
				if(!empty($_POST['db_hostname']) AND !empty($_POST['db_dbname']) AND !empty($_POST['db_username']) AND !empty($_POST['db_prefix']) AND !empty($_POST['admin_name']) AND !empty($_POST['admin_username']) AND !empty($_POST['admin_password']) AND !empty($_POST['admin_email']) AND !empty($_POST['admin_gbemail'])) {
					...
					if(isset($link) AND ($link == TRUE)) {
						...
						$sql[9] = "SELECT * FROM ".$_POST['db_prefix']."user";

						$link = mysqli_connect($_POST['db_hostname'], $_POST['db_username'], $_POST['db_password'], $_POST['db_dbname']) or die ("(install.php) Error, line 278: ".mysqli_error($link));
						mysqli_set_charset($link, 'utf8');

						foreach($sql as $val) {
							if(mysqli_query($link, $val) == TRUE) {
			...
			if($success == $to) {
				// MGB was successfully installed
