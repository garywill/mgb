<?php
	...
	*/
	...
	$sqldescription[39] = "- Adding 'sendmail_contactmail_text_copy' in settings table...";

	include("../language/".$settings['language_path']."/lang_admin.php");

	$sql[40] = "UPDATE `".$db['prefix']."settings` SET
	...
	$sqldescription[40] = "- Inserting values in new fields ...";
	$sqlisinsert[40] = 1;

	$sql[41] = "ALTER TABLE `".$db['prefix']."user` ADD
			`user_ip` VARCHAR( 15 ) NOT NULL AFTER `user_key`";
	$sqldescription[41] = "- Adding 'user_ip' in user table...";

	// 0.6.9.5

	$sql[42] = "CREATE TABLE IF NOT EXISTS ".$db['prefix']."spam (
			  `ID` INT NOT NULL AUTO_INCREMENT PRIMARY KEY ,
	...
	$sqldescription[42] = "- Creating spam table ...";

	$sql[43] = "CREATE TABLE IF NOT EXISTS ".$db['prefix']."banlist_ips (
			  `ID` INT NOT NULL AUTO_INCREMENT PRIMARY KEY ,
	...
	$sqldescription[43] = "- Creating ip banlist ...";

	$sql[44] = "CREATE TABLE IF NOT EXISTS ".$db['prefix']."banlist_emails (
			  `ID` INT NOT NULL AUTO_INCREMENT PRIMARY KEY ,
	...
	$sqldescription[44] = "- Creating email banlist ...";

	$sql[45] = "CREATE TABLE IF NOT EXISTS ".$db['prefix']."banlist_domains (
			  `ID` INT NOT NULL AUTO_INCREMENT PRIMARY KEY ,
			  `banned_domain` VARCHAR( 255 ) NOT NULL ,
			  `matches` INT( 11 ) NOT NULL ,
			  `timestamp` INT( 11 ) NOT NULL
			  )";
	$sqldescription[45] = "- Creating domain banlist ...";

	$sql[46] = "CREATE TABLE IF NOT EXISTS ".$db['prefix']."spam_log (
			  `ID` INT NOT NULL AUTO_INCREMENT PRIMARY KEY ,
	...
	$sqldescription[66] = "- Adding smtp fields in settings table...";

	$sql[67] = "ALTER TABLE `".$db['prefix']."settings` ADD `keystroke` TINYINT( 1 ) NOT NULL DEFAULT '1' AFTER `time_lock_spam_count` ,
	...
	$sqldescription[67] = "- Adding keystroke fields in settings table...";

	$sql[68] = "ALTER TABLE `".$db['prefix']."settings` ADD `captcha_max_length` TINYINT( 1 ) NOT NULL AFTER `captcha_length`";
	$sqldescription[68] = "- Adding field for captcha max length in settings table ...";

	$sql[69] = "ALTER TABLE `".$db['prefix']."settings` ADD `captcha_salt` VARCHAR( 255 ) NOT NULL DEFAULT '".mt_rand()."' AFTER `captcha_max_length`";
	$sqldescription[69] = "- Adding field for captcha salt in settings table ...";

	$sql[70] = "ALTER TABLE `".$db['prefix']."settings` ADD `captcha_hash_method` VARCHAR( 255 ) NOT NULL DEFAULT 'sha256' AFTER `captcha_salt`";
	$sqldescription[70] = "- Adding field for captcha hash method in settings table ...";

	$sql[71] = "ALTER TABLE `".$db['prefix']."settings` ADD `caching` TINYINT( 1 ) NOT NULL DEFAULT '0' AFTER `admin_gbemail`";
	$sqldescription[71] = "- Adding field for caching in settings table ...";

	$sql[72] = "ALTER TABLE `".$db['prefix']."settings` ADD `dynamic_fieldnames` TINYINT( 1 ) NOT NULL DEFAULT '1' AFTER `keystroke_ban_time`";
	$sqldescription[72] = "- Adding field for dynamic fieldnames in settings table ...";

	$sql[73] = "ALTER TABLE `".$db['prefix']."settings` ADD `dynamic_fieldnames_method` TINYINT( 1 ) NOT NULL DEFAULT '1' AFTER `dynamic_fieldnames`";
	$sqldescription[73] = "- Adding field for dynamic fieldnames method in settings table ...";

	$sql[74] = "ALTER TABLE `".$db['prefix']."settings` ADD `dynamic_fieldnames_length` INT( 255 ) NOT NULL DEFAULT '16' AFTER `dynamic_fieldnames_method`";
	$sqldescription[74] = "- Adding field for dynamic fieldnames length in settings table ...";

	// update version number every time
	if(isset($_POST['update_version']) AND $_POST['update_version'] == 1) {
		$sql[75] = "UPDATE `".$db['prefix']."settings` SET `version` = '".MGB_VERSION."'";
		$sqldescription[75] = "- Updating version number...";

		if(isset($settings['language_path']) AND $settings['language_path'] == "lang_german_ansi") {
			$sql[76] = "UPDATE `".$db['prefix']."settings` SET `language_path` = 'lang_german_utf8'";
			$sqldescription[76] = "- Updating language path (delete \"language/lang_german_ansi/\" after this upgrade and VERY IMPORTANT: start <a href='convert_ansi.php'>convert_ansi.php</a>)!";
		}
	} else {
		if(isset($settings['language_path']) AND $settings['language_path'] == "lang_german_ansi") {
			$sql[75] = "UPDATE `".$db['prefix']."settings` SET `language_path` = 'lang_german_utf8'";
			$sqldescription[75] = "- Updating language path (delete \"language/lang_german_ansi/\" after this upgrade and VERY IMPORTANT: start <a href='convert_ansi.php'>convert_ansi.php</a>)!";
		}
	}
?>
