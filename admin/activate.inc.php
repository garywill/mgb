	...
	if(!defined('ADMINISTRATION')) {
		...
		if(check_rights($_GET['action'], $_SESSION['ID'])) {
			...
			if(isset($_POST['dropbox']) AND $_POST['dropbox'] == 1) { // Activate all entries at once
				...
				$spam_list_result = mgb_sql_connect("SELECT COUNT(ID) FROM ".$db['prefix']."spam", "Error while counting entries in spam table.", 1);
				$spam_list_total = @mysqli_result($spam_list_result, 0);
				...
				for($i = 0; $i < mysqli_num_rows($spam_list_result); $i++) {
					$spam_list[$i] = mysqli_fetch_array($spam_list_result); // put all entries from spam table into an array named $spam_list
				}
				...
				for($i = 0; $i < mysqli_num_rows($unchecked_entries_result); $i++) {
					$store_spam = 1;
					$new_spam[$i] = mysqli_fetch_array($unchecked_entries_result);
						...
						if($spam_list[$j]['ip'] == $new_spam[$i]['ip'] OR $spam_list[$j]['email'] == $new_spam[$i]['email']) {
							...
							if ($spam_list[$j]['counter'] < 5) {
								...
								for($k = 0; $k < mysqli_num_rows($spam_list_result); $k++) {
									$spam_list[$k] = mysqli_fetch_array($spam_list_result); // put all entries from spam table into an array named $spam_list
								}
							}
						}
					}
					...
					if($store_spam == 1) {
						...
						for($l = 0; $l < mysqli_num_rows($whole_spam_entry); $l++) {
							$whole_entry[$l] = mysqli_fetch_array($whole_spam_entry);
			...
			if(isset($_GET['id'])) {
				if(isset($_GET['isspam']) AND secure_value($_GET['isspam'] == 1)) {
					...
					for($i = 0; $i < mysqli_num_rows($result); $i++) {
						$spam[$id] = mysqli_fetch_array($result);
					}
