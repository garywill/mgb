		...
		if(check_rights($_GET['action'], $_SESSION['ID']))
			...
			if(!empty($_POST['dropbox']))
				...
				elseif($_POST['dropbox'] == 8) // export as sql dump
					...
					// banlist_domains
					/* $sql_dump.= "--\n";
					$sql_dump.= "-- Structure of: banlist_domains\n";
					$sql_dump.= "--\n\n"; */

					$sql_dump.= "CREATE TABLE IF NOT EXISTS `{DB_PREFIX}banlist_domains` (
					...
					/* $sql_dump.= "--\n";
					$sql_dump.= "-- Data of: banlist_domains\n";
					$sql_dump.= "--\n\n"; */

					$result = mgb_sql_connect("SELECT banned_domain, timestamp FROM ".$db['prefix']."banlist_domains ORDER BY banned_domain ASC", "Error while loading data from banlist_domains for sql export.", 1);
					if(mysqli_num_rows($result) < 2)
						...
						for($i = 0; $i < mysqli_num_rows($result); $i++)
							...
							$ID = $i + 1;
							$sql_dump.= "INSERT INTO `{DB_PREFIX}banlist_domains` VALUES(".secure_value($ID).", ".secure_value($export[$i]['banned_domain']).", ".secure_value($export[$i]['timestamp']).");\n";
							}
						}
					else
						{
						$sql_dump.= "INSERT INTO `{DB_PREFIX}banlist_domains` VALUES\n";
						for($i = 0; $i < mysqli_num_rows($result); $i++)
							{
							$export[$i] = mysqli_fetch_array($result);
							$ID = $i + 1;
							$sql_dump.= "(".secure_value($ID).", ".secure_value($export[$i]['banned_domain']).", ".secure_value($export[$i]['timestamp']).")";
					...
					if(file_exists("../save") AND is_writable("../save"))
						{
						$timestamp = time();
						if(mgb_write_export_file("../save/".$timestamp."-".$db['prefix']."banlist_domains.sql", $sql_dump) === TRUE)
							{
							$script_time_end = microtime(true);
							$script_time = $script_time_end - $script_time_start;
							$template_message = "<span class='newer_version'><a href='../save/".$timestamp."-".$db['prefix']."banlist_domains.sql' target='_blank'>SQL Dump</a> erfolgreich in ".round($script_time, 3)." Sekunden erstellt!</span>";
							}
						else
							{
			...
			if ($_GET['p'] == 1)
				{
				$sf_forwards = "<a class=\"admin\" href=\"admin.php?action=banlist_domains&amp;p=".($_GET['p'] + 1).$sid."\" title=\"".$lang['page_forwards']."\">".$lang['page_forwards_symbol']."</a>";
				$sf_pagenumber = $_GET['p'];
				if ($pages_total >= 3 )
					{
					$sf_last = "<a class=\"admin\" href=\"admin.php?action=banlist_domains&amp;p=".$pages_total."\" title=\"".$lang['page_last']."\">".$lang['page_last_symbol']."</a>";
					}
				}
			...
			if ($_GET['p'] > 1)
				...
				if (($pages_total >= 3) AND ($_GET['p'] > 2))
					{
					$sf_first = "<a class=\"admin\" href=\"admin.php?action=banlist_domains&amp;p=1".$sid."\" title=\"".$lang['page_first']."\">".$lang['page_first_symbol']."</a>";
					}
				$sf_backwards = "<a class=\"admin\" href=\"admin.php?action=banlist_domains&amp;p=".($_GET['p'] - 1).$sid."\" title=\"".$lang['page_backwards']."\">".$lang['page_backwards_symbol']."</a>";
				$sf_pagenumber = $_GET['p'];
				$sf_forwards = "<a class=\"admin\" href=\"admin.php?action=banlist_domains&amp;p=".($_GET['p'] + 1).$sid."\" title=\"".$lang['page_forwards']."\">".$lang['page_forwards_symbol']."</a>";
				if (($pages_total >= 3) AND ($_GET['p'] < ($pages_total - 1)))
					{
					 $sf_last = "&nbsp;<a class=\"admin\" href=\"admin.php?action=banlist_domains&amp;p=".$pages_total.$sid."\" title=\"".$lang['page_last']."\">".$lang['page_last_symbol']."</a>";
					}
				}
			...
			$result = mgb_sql_connect("SELECT id, banned_domain, matches, timestamp FROM ".$db['prefix']."banlist_domains ORDER BY banned_domain ASC LIMIT $load_start, $load_end", "Error while loading banned domain entries.", 1);
