<?php
	...
	*/
	...
	if(isset($_SESSION['key'])) {
		if(!check_session($_SESSION['ID'], $_SESSION['key'], $_SESSION['ip'], $settings['session_timeout'])) {
			session_unset();
			session_destroy();
			$_SESSION = array();
	...
	// login status
	if(empty($_SESSION['ID']) OR !empty($_GET['action']) AND $_GET['action'] == "logout") {
		$include = "login.inc.php";
		$new_version_available = "&nbsp;";
	}
	else {
		...
		$mysqli_version = substr($version, 0, 7);

		$login_status_text = "<b>PHP</b>: ".phpversion()."&nbsp;| <b>MySQL</b>: ".$mysqli_version."&nbsp;|&nbsp;".$lang['logged_in']."&nbsp;|&nbsp;<a class='admin' href='admin.php?action=logout' title='{LANG_LOGOUT}'>{LANG_LOGOUT}</a>&nbsp;";
	...
	$page_navigation = template("LINK_FB", "http://www.facebook.com/MgbOpenSourceGuestbook", $page_navigation);
	...
	} elseif($_GET['action'] == 'banlist_ips' OR $_GET['action'] == 'banlist_emails' OR $_GET['action'] == 'banlist_domains' OR $_GET['action'] == 'spam_log') {
		$page_admin = template("TEMPLATE_DROPBOX_NORMAL", '', $page_admin);
		$page_admin = template("TEMPLATE_DROPBOX_SPAM", '', $page_admin);
		if($total >= 1) {
	...
	$page_admin = mgb_template_language($page_admin, "../language/".$settings['language_path']."/lang_admin.php", $settings['debug_mode']); // last number defines debug mode
